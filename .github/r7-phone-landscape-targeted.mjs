...
const landscapes = [['800x360',800,360],['844x390',844,390],['932x430',932,430]];
const portraits = [['390x844',390,844],['430x932',430,932]];
const results = [];
const screenshots = [];
...
async function railState(page, contentSelectors) {
  return page.evaluate(selectors => {
    ...
    for (const selector of selectors) {
      for (const el of document.querySelectorAll(selector)) {
        ...
        if (cs.display!=='none' && cs.visibility!=='hidden' && Number(cs.opacity||1)>0 && r.width>0 && r.height>0 && r.bottom>0 && r.top<innerHeight) {
          visibleRects.push({selector,left:r.left,right:r.right,top:r.top,bottom:r.bottom});
        }
      }
    }
    const contentRight=visibleRects.length ? Math.max(...visibleRects.map(r=>r.right)) : 0;
    return {
      ...
      horizontalProgressNodes:document.querySelectorAll('#scrollProgress').length,
      horizontalProgressVisible:[...document.querySelectorAll('.scroll-progress')].some(el=>getComputedStyle(el).display!=='none')
    };
  }, contentSelectors);
}
function railGeometryOk(s) {
  return !!s && s.display==='grid' && s.visibility!=='hidden' && Number(s.opacity)>0 &&
    s.rail.width<=21 && s.rail.left>=s.viewport.width-24 && s.rail.right<=s.viewport.width+.5 &&
    s.contentRight<=s.rail.left-2 && s.scrollWidth<=s.viewport.width+2;
}
...
for (const [engineName,launcher] of Object.entries(engines)) {
  ...
  for (const [vp,width,height] of landscapes) {
    ...
    add(engineName,vp,'CONTACT timing linear / no endpoint braking',contact.timing==='linear',JSON.stringify(contact));
    ...
    add(engineName,vp,'CONTACT no diamond/sweep/extra endpoint animation',contact.endpointDisplay==='none'&&contact.endpointAnimation==='none'&&contact.sweepDisplay==='none'&&contact.signalBeforeDisplay==='none',JSON.stringify(contact));

    if (vp==='844x390'||vp==='932x430') {
      const initial=await railState(page,['.hero h1','.hero-poster-frame','.hero-system-strip','.r7-mobile-hero-copy','.r7-mobile-hero-actions']);
      add(engineName,vp,'HOME system rail visible, edge-safe, no content overlap',railGeometryOk(initial),JSON.stringify(initial));
      ...
      add(engineName,vp,'HOME horizontal progress absent',initial?.horizontalProgressNodes===0&&!initial?.horizontalProgressVisible,JSON.stringify(initial));
    ...
    if (vp==='800x360') {
      ...
      await rp.locator('.review-composer').scrollIntoViewIfNeeded(); await rp.waitForTimeout(160); await shot(rp,engineName,vp,'reviews-rating');
    }

    if (vp==='844x390'||vp==='932x430') {
      const initial=await railState(rp,['.community-hero h1','.community-hero p','.community-metrics']);
      add(engineName,vp,'REVIEWS system rail visible, edge-safe, no content overlap',railGeometryOk(initial),JSON.stringify(initial));
      add(engineName,vp,'REVIEWS rail initial state RESPONSE',initial?.section.includes('R / RESPONSE'),JSON.stringify(initial));
      add(engineName,vp,'REVIEWS horizontal progress absent',initial?.horizontalProgressNodes===0&&!initial?.horizontalProgressVisible,JSON.stringify(initial));
  ...
  for (const [vp,width,height] of portraits) {
    ...
    add(engineName,vp,'PORTRAIT CONTACT still linear 4.15s',contact.timing==='linear'&&contact.duration==='4.15s'&&contact.name==='r7ContactTravel'&&contact.pointCount===1,JSON.stringify(contact));
...
const report={product_sha:PRODUCT_SHA,trigger_sha:process.env.GITHUB_SHA||null,engines:Object.keys(engines),landscapes:landscapes.map(([name,width,height])=>({name,width,height})),portraits:portraits.map(([name,width,height])=>({name,width,height})),totals,results,screenshots};
await fs.writeFile(path.join(OUT,'RESULTS.json'),JSON.stringify(report,null,2));
const failures=results.filter(r=>r.status==='FAIL');
await fs.writeFile(path.join(OUT,'REPORT.md'),['# R7 PHONE-LANDSCAPE TARGETED QA','',`Product SHA: ${PRODUCT_SHA}`,`Trigger SHA: ${process.env.GITHUB_SHA||'unknown'}`,'',`TOTAL ${totals.total} / PASS ${totals.pass} / FAIL ${totals.fail}`,'','## Failures',...(failures.length?failures.map(r=>`- ${r.engine} ${r.viewport} — ${r.check}: ${r.detail}`):['- none']),'','## Screenshots',...screenshots.map(s=>`- screenshots/${s}`)].join('\n'));
