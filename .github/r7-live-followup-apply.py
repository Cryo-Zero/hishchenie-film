...
# Temporary QA/meta commits are allowed, but runtime must still equal production baseline.
subprocess.run(["git", "diff", "--exit-code", BASE, "HEAD", "--", *RUNTIME_PATHS], check=True)

# Shared Actors/FAQ reveal: preserve geometry, slow the reversible record draw.
p = Path("js/site.js")
s = p.read_text()
old = """  // Keep the old state visible while it softens. The new state is written
...
"""
new = """  // Reverse-draw the current record first. The semantic value swap happens
  // only after that information is visually hidden; the new record then draws back in.
  // This keeps structural geometry stationary while making both directions legible.
  panel.classList.add('is-r7-crossfade-out');
  panel._theftRevealTimer = setTimeout(() => {
    if (typeof writeValues === 'function') writeValues();
    panel.classList.remove('is-r7-crossfade-out');
...
# Mobile Actors/FAQ: remove legacy min-height and scrollTo forcing. Keep semantic detail
# alive only for the reverse reveal, then switch to real index geometry.
p = Path("js/responsive-r7.js")
s = p.read_text()
a = s.index("  // Sequential FAQ / Actors --------------------------------------------------")
b = s.index("  // PROFILE HELP -------------------------------------------------------------")
replacement = r'''  // Sequential FAQ / Actors --------------------------------------------------
  // Follow-up rule: never force viewport coordinates or fabricate a temporary height.
  // Keep the real detail state alive only long enough for its reverse reveal, then switch
  // to the real index geometry. Browser scroll anchoring is disabled by follow-up CSS.
...
# Dossier footer follows the same back action; no timer-based scroll restoration.
...
# Load the bounded final layer on both public pages.
for name in ("index.html", "reviews.html"):
    p = Path(name)
    s = p.read_text()
    css = '<link rel="stylesheet" href="css/r7-owner-corrections.css?v=r7-owner-20260916b">'
    assert s.count(css) == 1, (name, s.count(css))
    s = s.replace(css, css + '\n<link rel="stylesheet" href="css/r7-live-followup.css?v=20260917a">')
    js = '<script src="js/r7-owner-corrections.js?v=r7-owner-20260916b"></script>'
    assert s.count(js) == 1, (name, s.count(js))
    s = s.replace(js, js + '\n<script src="js/r7-live-followup.js?v=20260917a"></script>')
    p.write_text(s)
...
.video-shell.r7-followup-player .r7-player-controls{padding:48px 16px 12px!important;background:linear-gradient(to top,rgba(2,5,6,.92),rgba(2,5,6,.52) 55%,transparent)!important}.r7-player-progress-row{margin-bottom:6px!important}.r7-player-row{gap:14px!important}.r7-player-left,.r7-player-right{gap:6px!important}.r7-player-button{width:40px!important;height:40px!important;min-width:40px!important;border:0!important;background:transparent!important;box-shadow:none!important;border-radius:2px}.r7-player-button:hover,.r7-player-button:focus-visible{border:0!important;background:rgba(155,196,199,.09)!important;box-shadow:none!important;outline:1px solid rgba(155,196,199,.38)!important;outline-offset:-3px}.r7-player-time{min-width:86px!important;color:#c1cecf!important}.r7-player-range{height:24px!important}.r7-player-range::-webkit-slider-runnable-track{height:2px!important;background:linear-gradient(90deg,#9bc4c7 0 var(--r7-range-fill,0%),rgba(155,196,199,.28) var(--r7-range-fill,0%) 100%)!important}.r7-player-range::-moz-range-track{height:2px!important;background:rgba(155,196,199,.28)!important}.r7-player-range::-moz-range-progress{height:2px!important;background:#9bc4c7!important}.r7-player-range::-webkit-slider-thumb{width:9px!important;height:9px!important;margin-top:-3.5px!important;background:#d4e4e5!important}.r7-player-range::-moz-range-thumb{width:9px!important;height:9px!important;background:#d4e4e5!important}.r7-player-volume{width:88px!important}.r7-player-glyph{width:16px!important;height:16px!important}
.video-shell:fullscreen,.video-shell:-webkit-full-screen{width:100vw!important;height:100vh!important;max-width:none!important;margin:0!important;padding:0!important;border:0!important;display:grid!important;place-items:center!important;overflow:hidden!important;background:#000!important}.video-shell:fullscreen video,.video-shell:-webkit-full-screen video{width:100vw!important;height:100vh!important;max-width:none!important;max-height:none!important;object-fit:contain!important;object-position:center!important}.video-shell:fullscreen .r7-player-controls,.video-shell:-webkit-full-screen .r7-player-controls{position:absolute!important;left:0!important;right:0!important;bottom:0!important;width:100%!important;display:block!important}
@media(max-width:820px){.video-shell.r7-followup-player .r7-player-controls{display:block!important;padding:34px 8px 7px!important}.video-shell.r7-followup-player .r7-player-row{gap:5px!important}.video-shell.r7-followup-player .r7-player-left,.video-shell.r7-followup-player .r7-player-right{gap:2px!important}.video-shell.r7-followup-player .r7-player-button{width:38px!important;height:38px!important;min-width:38px!important}.video-shell.r7-followup-player .r7-player-pip{display:none!important}.video-shell.r7-followup-player .r7-player-volume{display:none!important}.video-shell.r7-followup-player .r7-player-time{min-width:70px!important;font-size:8px!important;letter-spacing:0!important}.hero-inner{padding-bottom:34px!important}.hero-inner>.r7-mobile-hero-copy{margin-top:22px!important}.r7-mobile-hero-actions{margin-top:14px!important;margin-bottom:0!important}.review-composer .rating-scale{grid-template-columns:repeat(12,minmax(0,1fr))!important;gap:6px!important}.review-composer .rating-button{grid-column:span 2!important;width:100%!important;min-width:0!important;height:42px!important;min-height:42px!important}.review-composer .rating-button:nth-child(7){grid-column:2/span 2!important}.review-composer .rating-button:last-child{grid-column:span 2!important}}
body.r7-phone-landscape .hero-inner{padding-bottom:24px!important}body.r7-phone-landscape .r7-mobile-hero-copy{margin-top:14px!important}body.r7-phone-landscape .r7-mobile-hero-actions{margin-top:10px!important;margin-bottom:0!important}
.contact-signal-track i{animation:r7FollowupContactTravel 4.1s linear infinite!important}@keyframes r7FollowupContactTravel{0%{left:0;opacity:.1}10%{opacity:1}90%{opacity:1}100%{left:calc(100% - 5px);opacity:.1}}
@media(prefers-reduced-motion:reduce){.contact-signal-track i{animation:none!important;left:50%!important}.subject-dossier *,.faq-response-panel *{transition-duration:0s!important;transition-delay:0s!important}body.r7-sequential-ui #cast .cast-list,body.r7-sequential-ui #cast .subject-dossier,body.r7-sequential-ui #faq .faq-query-list,body.r7-sequential-ui #faq .faq-response-panel{animation:none!important}}
''')
...
Path("js/r7-live-followup.js").write_text(r'''(() => {
  ...
  function reorderNavigation(){
    ...
    const actors=links.find(a=>(a.getAttribute('href')||'').includes('#cast'));
    ...
    if(actors&&faq)actors.insertAdjacentElement('afterend',faq);
    if(faq&&reviews)faq.insertAdjacentElement('afterend',reviews);
    if(reviews&&report)reviews.insertAdjacentElement('afterend',report);
  }
  reorderNavigation();
  const video=$('#trailerVideo'),shell=$('#videoShell'),controls=shell?.querySelector('.r7-player-controls');
  if(!video||!shell||!controls)return;
  shell.classList.add('r7-followup-player');
  const progress=controls.querySelector('.r7-player-progress'),volume=controls.querySelector('.r7-player-volume'),pip=controls.querySelector('.r7-player-pip'),fullscreen=controls.querySelector('.r7-player-fullscreen');
  const compact=()=>matchMedia('(max-width:820px)').matches||(matchMedia('(pointer:coarse)').matches&&innerWidth>innerHeight&&innerWidth<=960&&innerHeight<=560);
  const fsSupported=()=>Boolean(shell.requestFullscreen||shell.webkitRequestFullscreen||video.webkitEnterFullscreen);
  const pipSupported=()=>Boolean((document.pictureInPictureEnabled&&video.requestPictureInPicture)||(video.webkitSupportsPresentationMode?.('picture-in-picture')&&video.webkitSetPresentationMode));
  function syncMode(){controls.hidden=false;video.controls=false;video.setAttribute('controlsList','nodownload noplaybackrate noremoteplayback');video.disableRemotePlayback=true;if(pip)pip.hidden=compact()||!pipSupported();if(fullscreen)fullscreen.hidden=!fsSupported();try{video.disablePictureInPicture=compact()}catch(_){}}
  function syncRanges(){if(progress){const p=Number(progress.value)||0;progress.style.setProperty('--r7-range-fill',`${Math.max(0,Math.min(100,p))}%`)}if(volume){const p=(Number(volume.value)||0)*100;volume.style.setProperty('--r7-range-fill',`${Math.max(0,Math.min(100,p))}%`)}}
  ['loadedmetadata','durationchange','timeupdate','seeked','volumechange'].forEach(n=>video.addEventListener(n,()=>requestAnimationFrame(syncRanges)));progress?.addEventListener('input',syncRanges);volume?.addEventListener('input',syncRanges);
  async function toggleFullscreen(){try{if(document.fullscreenElement||document.webkitFullscreenElement){const exit=document.exitFullscreen||document.webkitExitFullscreen;if(exit)await exit.call(document);return}if(shell.requestFullscreen)await shell.requestFullscreen();else if(shell.webkitRequestFullscreen)await shell.webkitRequestFullscreen();else if(video.webkitEnterFullscreen)video.webkitEnterFullscreen()}catch(_){}}
  shell.addEventListener('dblclick',e=>{if(compact()||e.target.closest('.r7-player-controls'))return;e.preventDefault();toggleFullscreen()});
  let engaged=false;shell.addEventListener('pointerdown',()=>{engaged=true});shell.addEventListener('focusin',()=>{engaged=true});document.addEventListener('pointerdown',e=>{if(!shell.contains(e.target))engaged=false},true);document.addEventListener('focusin',e=>{engaged=shell.contains(e.target)},true);
  document.addEventListener('keydown',e=>{if(compact()||e.key.toLowerCase()!=='f'||e.altKey||e.ctrlKey||e.metaKey)return;const t=e.target;if(t instanceof HTMLElement&&(t.matches('input,textarea,select,button,a[href],[contenteditable="true"]')||t.isContentEditable))return;if(!engaged&&!shell.contains(document.activeElement))return;e.preventDefault();toggleFullscreen()});
  addEventListener('resize',()=>{syncMode();syncRanges()},{passive:true});addEventListener('orientationchange',()=>requestAnimationFrame(()=>{syncMode();syncRanges()}),{passive:true});document.addEventListener('fullscreenchange',syncMode);document.addEventListener('webkitfullscreenchange',syncMode);syncMode();syncRanges();
})();
''')

for js in ["js/site.js", "js/responsive-r7.js", "js/r7-live-followup.js"]:
    subprocess.run(["node", "--check", js], check=True)

allowed = {"index.html", "reviews.html", "js/site.js", "js/responsive-r7.js", "js/r7-live-followup.js", "css/r7-live-followup.css"}
changed = set(subprocess.check_output(["git", "diff", "--name-only"], text=True).split())
assert changed == allowed, (changed, allowed)
assert "scrollTo({ left: x, top: y" not in Path("js/responsive-r7.js").read_text()
assert "setTimeout(restore" not in Path("js/responsive-r7.js").read_text()
