...
- Supabase unique alias index is applied and verified for R6 backend.
...
6. With the R6 UNIQUE index active, no duplicate canonical alias identity can be persisted for rows covered by the predicate.
...
- Supabase project `xltwwvutqkpmtmlavngi`: `ACTIVE_HEALTHY` at verification.
- Migration `20260908155140_revival_r6_unique_aliases`: **APPLIED**.
- `profiles_unique_alias_identity_idx`: **UNIQUE and verified** on `public.profiles`.
- Canonical duplicate groups after migration: **0**.
- User-data table counts were unchanged by the migration: profiles 16, reviews 11, review_likes 16, review_replies 6, admins 0.
- Backend blocker: **cleared**.
- Frontend state remains separate: PR #17 is still Draft and R6 is not merged/published yet.
