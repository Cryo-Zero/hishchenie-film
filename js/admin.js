(() => {
  ...
  async function loadReviews(){setStatus('Загрузка…');try{const rows=await rpc('admin_get_reviews_v1',{});render(Array.isArray(rows)?rows:[]);setStatus(`Отзывы: ${Array.isArray(rows)?rows.length:0}`)}catch(e){setStatus(`Не удалось загрузить: ${e.message}`,true)}}
  ...
  function render(rows){const root=$('#adminReviews');root.replaceChildren();if(!rows.length){const d=document.createElement('div');d.className='admin-card';d.textContent='Отзывов пока нет.';root.append(d);return}
    rows.forEach(r=>{const card=document.createElement('article');card.className='admin-card';const head=document.createElement('div');head.className='admin-card-head';const left=document.createElement('div');const n=document.createElement('strong');n.textContent=r.display_name||'Anonymous';const meta=document.createElement('div');meta.className='composer-label';meta.textContent=`${r.rating}/10 · ${new Date(r.created_at).toLocaleString('ru-RU')} · ♥ ${r.like_count||0} · ↳ ${r.reply_count||0}`;left.append(n,meta);const flags=document.createElement('div');flags.className='composer-label';flags.textContent=`${r.is_pinned?'PINNED ':''}${r.is_hidden?'HIDDEN':''}`.trim()||'PUBLIC';head.append(left,flags);const p=document.createElement('p');p.textContent=r.review_text||'Оценка без текста.';const actions=document.createElement('div');actions.className='admin-actions';
