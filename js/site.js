(() => {
  ...
  function syncLBDots(){if(!lightbox)return;let d=$('.p22-lightbox-dots',lightbox);if(!d){d=document.createElement('div');d.className='p22-lightbox-dots';lightbox.append(d);}d.replaceChildren(...items.map((_,i)=>{const x=document.createElement('i');if(i===ai)x.className='active';return x;}));}
